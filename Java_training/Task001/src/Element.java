...
public enum Element {
    H("Hydrogen",1,1.0008f),HE("Helium", 2,4.0026f),Ne("Neon", 10,20.180f);

    private static final Map<String, Element> BY_LABEL = new HashMap<>();
    private static final Map<Integer, Element> BY_AUTOMIC_NUMBER = new HashMap<>();
    private static final Map<Float, Element> BY_ATOMIC_WEIGHT =  new HashMap<>();

    static{
        for(Element e: values()){
            BY_LABEL.put(e.label,e);
            BY_AUTOMIC_NUMBER.put(e.atomicNumber, e);
    ...
    public static Element valueOfAtomicNumber(int number){
        return BY_AUTOMIC_NUMBER.get(number);
    }
